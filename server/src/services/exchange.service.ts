...
export class ExchangeService {
    ...
    getTicker(exchange: Exchange, symbol: string): Observable<any> {
        switch (exchange.toLowerCase()) {
            case Exchange.poloniex:
                return this.getFromCache(`${Exchange.poloniex}_ticker_${symbol}`);
            case Exchange.kraken:
                return this.getFromCache(`${Exchange.kraken}_ticker_${symbol}`);
            case Exchange.bittrex:
                return this.getFromCache(`${Exchange.bittrex}_ticker_${symbol}`);
            case Exchange.bitfinex:
                return this.getFromCache(`${Exchange.bitfinex}_ticker_${symbol}`);
            default:
                return Observable.of([`${exchange} not yet implemented`]);
        }
    }
